...
def main():
    immunes, infections = parse_file("input/24.txt")
    # immunes, infections = parse_file("input/24.test")

    while len(immunes) > 0 and len(infections) > 0:
        print()
        pprint.pprint(immunes)
        pprint.pprint(infections)
    ...
    print(sum([x.units for x in immunes]))
    print(sum([x.units for x in infections]))
